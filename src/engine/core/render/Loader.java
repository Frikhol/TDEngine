...
public class Loader {
    ...
    private static Mesh loadToVAO(float[] positions, int[] indices, float[] textureCoords, float[] normals, ArrayList<Vector3f> vertices){
        int vaoID = createVAO();
        ...
        unbindVAO();
        return new Mesh(vaoID,vboList,vertices,indices.length);
    }
    ...
    public static Mesh loadObjModel (String filename){
        ...
        return loadToVAO(verticesArray,indicesArray,textureArray,normalsArray, vertices);
    }
