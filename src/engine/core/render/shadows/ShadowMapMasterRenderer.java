...
/**
 * This class is in charge of using all of the classes in the core.render.shadows package to
 * carry out the shadow render pass, i.e. rendering the scene to the shadow map
 * texture. This is the only class in the core.render.shadows package which needs to be
 * referenced from outside the core.render.shadows package.
 * 
 * @author Karl
 *
 */
public class ShadowMapMasterRenderer {
	...
	public void render(Map<Model, List<GameObject>> entities, Light sun) {
		shadowBox.update();
	...
	public Matrix4f getToShadowMapSpaceMatrix() {
		return new Matrix4f(offset).mul(new Matrix4f(projectionViewMatrix));
	}
	 ...
	 * core.render.shadows FBO so that everything rendered after this gets rendered to the
	...
	private void prepare(Vector3f lightDirection, ShadowBox box) {
		...
		new Matrix4f(projectionMatrix).mul(lightViewMatrix, projectionViewMatrix);
	...
	private void updateLightViewMatrix(Vector3f direction, Vector3f center) {
		...
		new Matrix4f(lightViewMatrix).rotate(pitch, new Vector3f(1, 0, 0), lightViewMatrix);
		float yaw = (float) Math.toDegrees(((float) Math.atan(direction.x / direction.z)));
		yaw = direction.z > 0 ? yaw - 180 : yaw;
		new Matrix4f(lightViewMatrix).rotate((float) -Math.toRadians(yaw), new Vector3f(0, 1, 0),lightViewMatrix);
		new Matrix4f(lightViewMatrix).translate(center,lightViewMatrix);
	}
